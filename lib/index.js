...
NefitEasyClient.prototype.ping = function() {
  var self = this;
  void function ping() {
    debug('sending ping');
    ...
    // quick "fix" unref function doesn work in Electron/Chromium
    setTimeout(ping, self.opts.pingInterval);//.unref();
  }();
};
...
NefitEasyClient.prototype.on = function() {
  return this.client.on.apply(this.client, arguments);
};

NefitEasyClient.prototype.send = function(message) {
  // Queue the request
...
NefitEasyClient.prototype.buildMessage = function(body) {
...
NefitEasyClient.prototype.get = function(uri) {
  var message = this.buildMessage(`GET ${ uri } HTTP/1.1\rUser-Agent: NefitEasy\r\r`);
